...
def process_statement(pdf_path: Path, password: str, job_dir: Path, contas_path: Path) -> dict:
    accounts = extract_accounts(contas_path)
...
async def processar_extrato(
    ...
    plano_contas: Optional[UploadFile] = File(None),
    senha: str = Form(""),
) -> dict:
    ...
    try:
        uploaded = save_upload_file(arquivo, job_dir, "extrato")
        contas_path = CONTAS
        if plano_contas and plano_contas.filename:
            contas_path = save_upload_file(plano_contas, job_dir, "plano_contas")

        result = process_statement(uploaded, senha, job_dir, contas_path)
